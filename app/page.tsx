...
export default function Home(){
  ...
  return <main className={`game-shell ${traveling?"is-traveling":""}`}>
    ...
    {started&&<><div className="controls" aria-label="Movement controls"><span>Move · Drag to orbit · Scroll to zoom · V to change view</span><div><kbd>W</kbd></div><div><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd></div></div>
    ...
    {nearby&&<button className="discover" onClick={()=>beginTravel(nearby)}><small>Destination discovered</small><strong>{destinations.find(d=>d.id===nearby)?.label}</strong><span>Step through →</span></button>}</>}
